...
public:
    ...
    mapOptimization()
    {
        ...
        // euigon
        pubGpsPath                  = nh.advertise<nav_msgs::Path>("/liosam_path", 1);

        subCloud = nh.subscribe<sensor_fusion::cloud_info>("lio_sam/feature/cloud_info", 1, &mapOptimization::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());
        // subGPS   = nh.subscribe<nav_msgs::Odometry> (gpsTopic, 200, &mapOptimization::gpsHandler, this, ros::TransportHints().tcpNoDelay());
        subLoop  = nh.subscribe<std_msgs::Float64MultiArray>("lio_loop/loop_closure_detection", 1, &mapOptimization::loopInfoHandler, this, ros::TransportHints().tcpNoDelay());

        // subscribe uv measurements
        subUVMeas = nh.subscribe<sensor_msgs::PointCloud>("vio/data_uv", 500, &mapOptimization::uvHandler, this, ros::TransportHints().tcpNoDelay());
    ...
    void laserCloudInfoHandler(const sensor_fusion::cloud_infoConstPtr& msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            updateInitialGuess(); // Tk+1(init_guess) or initialize with imu orientation

            // laserCloudCornerFromMap is cleared at the start
            // Find surrounding key poses and register the features from the key poses to the local map (laserCloudCornerFromMap)
            extractSurroundingKeyFrames(); // Qk

            // downsample the feature pointcloud just received
            downsampleCurrentScan(); // Pk+1 (bar)

            scan2MapOptimization(); // Qk+1(bar)과 Qk matching을 통한 Tk+1 보정 

            saveKeyFramesAndFactor(); // iSAM으로 Pose optimization based on closed loop and GPS & save keyframes' features for icp and mapping

            correctPoses(); // clopsed Loop에 대한 pose correction
    ...
    bool saveMapService(sensor_fusion::save_mapRequest& req, sensor_fusion::save_mapResponse& res)
    {
    ...
    void visualizeGlobalMapThread()
    {
        ...
        sensor_fusion::save_mapRequest  req;
        sensor_fusion::save_mapResponse res;
    ...
    void updateInitialGuess()
    {
        ...
        // initialGuess는 odometryQueue(XYZRPY) 중에 timeScanCur에 가장 가까운 odom 값

        if (cloudInfo.odomAvailable == true)
        {
            //startOdomMsg (timeScanStart와 가장 가까운 odometry)
            ...
            if (lastImuPreTransAvailable == false)
            {
                ...
                pcl::getTranslationAndEulerAngles(transFinal, transformTobeMapped[3], transformTobeMapped[4], transformTobeMapped[5], 
                                                              transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]); // transFinal (Tk+1)

                lastImuPreTransformation = transBack;
                // save imu before return (새로운 sweep 시작할 때의 imu orientation)
        ...
        // odometry가 available하지 않을 때는 lastImuTransformation이 새로운 sweep을 시작할 때의 Imu rotation으로 사용한다.
        if (cloudInfo.imuAvailable == true)
        {
            Eigen::Affine3f transBack = pcl::getTransformation(0, 0, 0, cloudInfo.imuRollInit, cloudInfo.imuPitchInit, cloudInfo.imuYawInit); // Scan 시작할 때의 imu orientation
    ...
    // input: pointCloud이긴 하지만 robot의 key 3D pose 들임 
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)
    {
        // fuse the map
        laserCloudCornerFromMap->clear();
        laserCloudSurfFromMap->clear(); 
        for (int i = 0; i < (int)cloudToExtract->size(); ++i) // 주변 poses들 중에 가까운 pose 찾기
        {
            ...
            // 주변 Pose에서의 corner, surf가 global frame으로 transform 돼서 map container에 들어 있다면
            if (laserCloudMapContainer.find(thisKeyInd) != laserCloudMapContainer.end()) 
            {
                ...
                *laserCloudSurfFromMap   += laserCloudMapContainer[thisKeyInd].second;
                ...
                // 주변 Pose에서의 corner, surf가 global frame으로 transform 되지 않았다면 
                // transformed cloud not available
                pcl::PointCloud<PointType> laserCloudCornerTemp = *transformPointCloud(cornerCloudKeyFrames[thisKeyInd],  &cloudKeyPoses6D->points[thisKeyInd]); // key pose위치에서 찍힌 corner들 global frame으로 Transform
                pcl::PointCloud<PointType> laserCloudSurfTemp = *transformPointCloud(surfCloudKeyFrames[thisKeyInd],    &cloudKeyPoses6D->points[thisKeyInd]); // key pose위치에서 찍힌 surface들 global frame으로 Transform
                *laserCloudCornerFromMap += laserCloudCornerTemp; // laserCloudCornerFromMap에 추가
                *laserCloudSurfFromMap   += laserCloudSurfTemp; // laserCloudSrufFromMap에 추가 
                laserCloudMapContainer[thisKeyInd] = make_pair(laserCloudCornerTemp, laserCloudSurfTemp); //laserCloudMapContainer에 추가 
            }
    ...
    void addUVFactor(double timestamp, std::vector<uint> ids, std::vector<Eigen::Vector2d> uvs)
    {
    ...
    void publishOdometry()
    {
        ...
        laserOdometryROS.pose.pose.position.x = transformTobeMapped[3]; // imu, map-optimization, iSAM optimization 한 후의 pose
        ...
        if (lastIncreOdomPubFlag == false)
        {
            ...
            // incrementalOdometryAffineFront: imu로 propagate와 map-optimize하기 전의 위치 = 이전 frame에서 모든 optimized가 된 위치 
            // incrementalOdometryAffineBack: imu propagate와 map-optimization한 후의 위치 (iSAM optimization 이전)
            // increOdomAffine: 처음 frame에서만 iSAM optimized된 위치
    ...
    void publishFrames()
    {
        ...
        // {
            gpsGlobalPath.header.stamp = timeLaserInfoStamp;
            gpsGlobalPath.header.frame_id = odometryFrame;
            pubGpsPath.publish(gpsGlobalPath);
        // }
    }
};
